...
replication:
  ...
  oplogSizeMB: 2048                     # Размер oplog в мегабайтах (рекомендуется 5-50% от свободного места на диске)
  enableMajorityReadConcern: true       # Разрешить использование readConcern "majority" для чтения подтверждённых данных
